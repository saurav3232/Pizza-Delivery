...
const AddOrUpdatePizza = (props) => {
  ...
  const pizzaObj = props.pizzaDetails;
  ...
  const [pizzaformData, setPizzaFormData] = useState({
    ...
    pizzaAvatar: pizzaObj.pizzaAvatar || "",
  ...
  const [pizzaPrice, setPizzaPrice] = useState({
    smallPizza: pizzaObj.price.smallPizza || 0,
    mediumPizza: pizzaObj.price.mediumPizza || 0,
    largePizza: pizzaObj.price.largePizza || 0,
  });
  ...
  const handleClose = async () => {
    ...
    const { status } = await axios.post(
      "http://localhost:5000/api/admin/manage-pizza/update-pizza/" +
        pizzaObj._id,
      pizzaformData,
      {
        headers: {
...
export default AddOrUpdatePizza;
